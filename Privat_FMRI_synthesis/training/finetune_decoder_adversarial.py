...
# Checkpoint to load or resume from
CHECKPOINT_PATH = "/home/jovyan/work/ssd0/USERS/sajbendaniel/fmri-diffusion/Privat_FMRI_synthesis/checkpoints_adversarial/best_finetuned_adversarial_autoencoder.pt"
# Initial autoencoder if not resuming
...
log_file = logs_dir / f"adversarial_training_{timestamp}.txt"
...
def adversarial_finetune_decoder():
    ...
    def validate():
        ...
        with torch.no_grad():
            for images, _ in tqdm(val_loader, desc="Validation", leave=False):
                try:
                    ...
                    # Total loss
                    total_loss = config['lambda_recon'] * recon_loss + current_lambda_adv * adv_loss
        ...
        if val_recon_losses:
            ...
            return avg_total_loss, avg_recon_loss, avg_adv_loss, avg_disc_loss
        else:
    ...
    logger.info(f"Starting adversarial fine-tuning from epoch {start_epoch+1} to {config['num_epochs']}")
    ...
    for epoch in range(start_epoch, config['num_epochs']):
        ...
        for batch_idx, (images, _) in enumerate(pbar):
            try:
                ...
                loss_recon = recon_criterion(recon_no_skips, images)
                ...
                # Combined loss
                loss_ae = config['lambda_recon'] * loss_recon + current_lambda_adv * loss_adv
                ...
                # Record losses
                train_recon_losses.append(loss_recon.item())
        ...
        # Run validation
        val_total, val_recon, val_adv, val_disc = validate()
        ...
        logger.info(f"Train - Recon: {avg_train_recon:.6f}, Adv: {avg_train_adv:.6f}, Total: {avg_train_total:.6f}")
        logger.info(f"Train - Disc Loss: {avg_train_disc:.6f}, Real Acc: {avg_train_real_acc:.4f}, Fake Acc: {avg_train_fake_acc:.4f}")
        logger.info(f"Val   - Recon: {val_recon:.6f}, Adv: {val_adv:.6f}, Total: {val_total:.6f}, Disc: {val_disc:.6f}")
        ...
        # Check if model improved - focus on reconstruction loss
        ...
        # Adjust adversarial weight if discriminator is too strong
        if avg_train_fake_acc > 0.9 and epoch > 5:
            current_lambda_adv *= 1.2
    ...
    logger.info("Adversarial fine-tuning completed!")
    logger.info(f"Total training time: {int(hours)}h {int(minutes)}m {int(seconds)}s")
    logger.info(f"Best reconstruction loss: {best_recon_loss:.6f}")
    
    return best_recon_loss

if __name__ == "__main__":
    best_loss = adversarial_finetune_decoder()
    logger.info(f"Final best reconstruction loss: {best_loss:.6f}")